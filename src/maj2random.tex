...
\noindent
\emph{maj2random} is a low-overhead floating-point hash function built
upon a reduced-round version of the SHA-2 hash algorithm. it retains
the high-quality mixing of the compression function to permute entropy from a
\emph{GLSL vec2}, producing two random values between 0.0 and 1.0. the name
\emph{maj2random} comes from the \emph{maj} mixing function and pays homage
to \emph{arc4random}, where the "2" refers to the number of rounds.
\\
\par\noindent
\emph{maj2random} trades cryptographic strength for speed, making it suitable
for GPU shaders, procedural textures, or simulations where high-quality
randomness is needed but where full SHA-256 would be too costly. it produces
values closely approximating a uniform distribution on [0,1], suitable for
ray tracing and shader functions. \emph{maj2random} uses only 2 rounds
with $\approx 3\%$ of the overhead of the full SHA-256 hashing function.
...
\mathrm{maj}(x,y,z) & = & \multicolumn{5}{l}{(x \land y) \oplus (x \land z) \oplus (y \land z)} \\[1em]
...
\sigma_1(x) & = & \mathrm{ror}(x,17) & \oplus & \mathrm{ror}(x,19) & \oplus & \mathrm{shr}(x,10) \\[1em]
