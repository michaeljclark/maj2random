...
static vec2 bench_maj2random(size_t n)
{
    vec2 r = { 0, 0 };
    auto t1 = high_resolution_clock::now();
    for (size_t i = 0; i < n; i++) {
        vec2 x = maj2random({ i / (float)n, i / (float)n });
        r.a[0] += x.a[0];
        r.a[1] += x.a[1];
    }
    auto t2 = high_resolution_clock::now();
    print_result("maj2random", n, t1, t2);
    return r;
}
...
{
    print_header();
    vec2 r = bench_maj2random(1000000);
    print_footer();
    printf("r={%f,%f}\n", r.a[0], r.a[1]);
}
